...
class SettingsPage {
    ...
    init() {
        ...
        // Diagnostics + content pack tools
        this.initDiagnosticsAndContentPack();
    }

    initDiagnosticsAndContentPack() {
        const refreshDiagnosticsBtn = document.getElementById('refresh-diagnostics');
        const diagnosticsSummary = document.getElementById('diagnostics-summary');
        const diagnosticsList = document.getElementById('diagnostics-client-list');

        const exportBtn = document.getElementById('export-content-pack');
        const importBtn = document.getElementById('import-content-pack');
        const importFile = document.getElementById('content-pack-file');
        const importMergeToggle = document.getElementById('content-import-merge');
        const importSettingsToggle = document.getElementById('content-import-settings');
        const contentPackStatus = document.getElementById('content-pack-status');

        const setContentPackStatus = (message, isError = false) => {
            if (!contentPackStatus) return;
            contentPackStatus.textContent = message;
            contentPackStatus.style.color = isError ? 'var(--color-error)' : '';
        };

        const renderDiagnosticsClients = (clients = [], staleAfterSeconds = 120) => {
            if (!diagnosticsList || !diagnosticsSummary) return;

            if (!Array.isArray(clients) || clients.length === 0) {
                diagnosticsSummary.textContent = 'No active clients.';
                diagnosticsList.innerHTML = '<p class="hint">No client snapshots yet.</p>';
                return;
            }

            diagnosticsSummary.textContent = `${clients.length} client${clients.length === 1 ? '' : 's'} active (stale after ${staleAfterSeconds}s)`;
            diagnosticsList.innerHTML = clients.map((client) => {
                ...
                return `
                    <div class="diag-card ${client.active ? 'diag-active' : 'diag-stale'}">
                        <div class="diag-header">
                            <strong>${client.sessionName || client.username || 'Unknown User'}</strong>
                            <span class="diag-age">${client.lastSeenAgoSeconds}s ago</span>
                        </div>
                        <div class="diag-meta">
        ...
        const refreshDiagnostics = async () => {
            if (!refreshDiagnosticsBtn) return;

            refreshDiagnosticsBtn.disabled = true;
            refreshDiagnosticsBtn.textContent = 'Refreshing...';

            try {
                const result = await API.diagnostics.clients({ background: true });
                renderDiagnosticsClients(result?.clients || [], result?.staleAfterSeconds || 120);
            } catch (err) {
                if (diagnosticsSummary) diagnosticsSummary.textContent = 'Diagnostics unavailable for this user.';
                if (diagnosticsList) diagnosticsList.innerHTML = `<p class="hint">${err.message || 'Failed to load diagnostics.'}</p>`;
            } finally {
                refreshDiagnosticsBtn.disabled = false;
                refreshDiagnosticsBtn.textContent = 'Refresh Clients';
            }
        };
        ...
        exportBtn?.addEventListener('click', async () => {
            ...
            try {
                const payload = await API.contentPack.export();
                ...
                setContentPackStatus(`Exported ${fileName}`);
            } catch (err) {
                setContentPackStatus(err.message || 'Failed to export content pack.', true);
            } finally {
        ...
        importFile?.addEventListener('change', async () => {
            ...
            try {
                setContentPackStatus('Importing content pack...');

                const text = await file.text();
                const pack = JSON.parse(text);

                const result = await API.contentPack.import({
                    pack,
                    mode: importMergeToggle?.checked ? 'merge' : 'replace',
                    includeSettings: importSettingsToggle?.checked !== false
                });

                setContentPackStatus(`Import complete. Sources: ${result?.imported?.sources || 0}, Hidden: ${result?.imported?.hiddenItems || 0}, Favorites: ${result?.imported?.favorites || 0}`);

                await this.app.sourceManager.loadSources();
                if (this.app.currentPage === 'live') {
                    await this.app.channelList.loadSources();
                    await this.app.channelList.loadChannels();
                }
            } catch (err) {
                setContentPackStatus(err.message || 'Failed to import content pack.', true);
            } finally {
                importFile.value = '';
            }
        });
